...
$dscPath = Get-ChildItem -Path "C:\Program Files\WindowsApps" -Recurse -Filter dsc.exe -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty FullName
...
$jsonPath = "C:\ProgramData\Microsoft\DevBoxAgent\ImageDefinitions\dsc-apply-config-catalog-01\dsc-apply-config\install-insomnia.dsc.json"
